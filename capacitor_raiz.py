...
#Capacitor Raiz (Exemplo 6.2)

import numpy as np

# ------------------------------------------------------------
# Exemplo 6.2: Cálculo da capacitância para interface f(x) = h*sqrt(x/W)
...
# ------------------------------------------------------------
# Função para combinar duas capacitâncias em série
...
# ------------------------------------------------------------
# Função para calcular a posição da interface raiz quadrada f(x)
# f(x) = h*sqrt(x/W), onde:
...
# ------------------------------------------------------------
def fr(d, n, i):
    return d * (i / n)**0.5  # Calcula a posição da interface
                             #  entre os dois dielétricos para a i-ésima fatia

# ------------------------------------------------------------
# Função para calcular a capacitância analítica, Eq. (15) do manuscrito
# Fórmula: C = 2*ε1*ε2*L*W*[(ε2 - ε1) + ε1*ln(ε1/ε2)]/[h*(ε1 - ε2)^2]
# ------------------------------------------------------------
def C_analytic_root(ε1, ε2, h, W, L):
    num = 2*ε1*ε2*L*W*((ε2 - ε1) + ε1*np.log(ε1/ε2)) # Numerador da Eq. analítica
    den = h*(ε1 - ε2)**2                             # Denominador da Eq. analítica
...
def main():
    ...
    C_num = C_numeric_root(ε1, ε2, h, W, L, N)

    # Cálculo da capacitância analítica
    C_th = C_analytic_root(ε1, ε2, h, W, L)
    ...
    # Exibição dos resultados em pF (1 F = 1e12 pF)
...
# ------------------------------------------------------------
def C_numeric_root(ε1, ε2, h, W, L, N):
